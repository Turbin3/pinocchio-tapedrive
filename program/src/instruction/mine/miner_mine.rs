...
use tape_api::{error::TapeError, pda::miner_pda, EMPTY_SEGMENT, MAX_CONSISTENCY_MULTIPLIER, MAX_PARTICIPATION_TARGET, MIN_CONSISTENCY_MULTIPLIER, MIN_MINING_DIFFICULTY, MIN_PARTICIPATION_TARGET, SEGMENT_PROOF_LEN};
use crate::{api::utils::{compute_challenge, compute_next_challenge}, state::{try_from_account_info_mut, Archive, Block, Epoch, Mine, Miner, PoA, PoW, Tape, ADJUSTMENT_INTERVAL, BLOCK_DURATION_SECONDS, EPOCH_BLOCKS}};
...
fn get_scaled_reward(reward: u64, multiplier: u64) -> u64 {
    ...
    reward.saturating_mul(multiplier).saturating_div(MAX_CONSISTENCY_MULTIPLIER)
}
...
fn adjust_participation(epoch: &mut Epoch){
    if epoch.duplicates == 0 {
        if epoch.number % ADJUSTMENT_INTERVAL == 0{
            epoch.target_participation = epoch.target_participation.saturating_add(1).min(MAX_PARTICIPATION_TARGET);
        }
    }else {
        epoch.target_participation = epoch.target_participation.saturating_sub(1).max(MIN_PARTICIPATION_TARGET);
    }
}
...
fn adjust_difficulty(epoch: &mut Epoch, current_time: i64) {
    ...
    if average_time_per_block < BLOCK_DURATION_SECONDS as i64 {
        ...
        epoch.mining_difficulty = epoch.mining_difficulty.saturating_sub(1).max(MIN_MINING_DIFFICULTY);
    }
}
